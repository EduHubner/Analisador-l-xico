...
while i < len(entrada):
    if entrada[i] in afd[estado]:
        estado = afd[estado][entrada[i]]
        lexema += entrada[i]
        pos += 1
        i += 1
    elif estado in finais:
        # Se o estado atual for um estado final, adiciona o token e reinicia
        if finais[estado] != "ESPACO":  # Ignora espaços em branco
            lista_tokens.append(Token(finais[estado], lexema, pos - len(lexema)))
        estado = "INICIAL"
        lexema = ""
    else:
